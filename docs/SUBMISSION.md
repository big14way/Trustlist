...
- **The cold start is fast only while the prebuilt binaries are reachable.** A machine that has never seen the repo used to compile the whole Rust workspace, which was 419 of the 471 seconds it took. It now downloads binaries built for that exact commit and comes up in 150 seconds. The download is refused unless the commit matches, the published checksum matches, and each binary runs and reports the commit it was built from, so the worst case is the old behaviour rather than a wrong one. The repository is public, so the download needs no credentials, which is the case that matters: a stranger following the README has none.
