...
# No token goes in here, deliberately. The repository is public, so the
# prebuilt binaries download anonymously, and a stranger following the README
# has no token either. Passing one would make this measure a path a stranger
# cannot take.
docker run --rm \
  --network host \
